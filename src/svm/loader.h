...
struct VMFunction *loadmodule(VMState vm, FILE *input);
  // Load the first module, if any from the given input file.
  // If there is no more input or all lines are blank, return NULL.
  // Otherwise read and load one module.
  ...
  //
  // The return value is allocated with `vmalloc` and will be garbage-collected.
...
Modules loadmodules(VMState vm, FILE *input); 
  // Load all the modules present in a given file.
