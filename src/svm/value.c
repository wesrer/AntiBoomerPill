...
bool eqvalue(Value v1, Value v2) { // Will not work for hashing!
  if (v1.tag != v2.tag)
    ...
    case DenseConsCell: return false;
    case Emptylist: return true;
...
bool eqtests(Value v1, Value v2) { // will not work for hashing!
  ...
  else if (v1.tag == v2.tag && v1.tag == DenseConsCell)
    return eqtests(v1.dense_cons->car, v2.dense_cons->car) &&
           eqtests(v1.dense_cons->cdr, v2.dense_cons->cdr);
  else
