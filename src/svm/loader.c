...
static struct VMFunction *loadfun(VMState vm, int arity, int count, FILE *input) {
  ...
  *maxregp = 0;
  ...
  struct VMFunction *fun = vmalloc(sizeof (*fun) + (count + 1) * sizeof(fun->instructions[0]));
  fun->arity = arity;
  fun->size = count + 1;
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (n == strtoname(".load")) {
      int reg = tokens_get_number(&tokens_left, buffer);
      //check for max regs here
      assert (tokens_get_name(&tokens_left, buffer) == strtoname("function"));
      int a = tokens_get_number(&tokens_left, buffer);
      int c = tokens_get_number(&tokens_left, buffer);
